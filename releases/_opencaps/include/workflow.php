...
if ($_GET['task'] == "print_projs") {
	...
	if (isset($_GET['page']))
		$this_proj->printRemoteProjects($_GET['rid'], $_GET['page']);
	else
		$this_proj->printRemoteProjects($_GET['rid']);
 ...
 */	
} else if ($_GET['task'] == 'save_json') {
	$this_proj->saveJson($_GET['json'], $_GET['pid']);	
...
/* start tabs */
	...
	if (isset($remote_systems)) { 
		echo '<ul>';
		if(!DISABLE_LOCAL)		
			echo '<li id="home"><a href="start.php">Home</a></li>';	
			
		foreach ($remote_systems as $key=>$remote) {
